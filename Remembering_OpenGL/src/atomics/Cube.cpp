...
Cube::Cube(int wWidth, int wHeight, float xpos, float ypos, float zpos)
...
void Cube::OnRender(glm::mat4 proj, glm::mat4 view, std::vector<glm::mat4> model_positions)
{
    m_Proj = proj;
    m_View = view;
    GLCall(glClearColor(0.2f, 0.5f, 0.5f, 1.0f));
    ...
    {
        //glm::mat4 model = translate * rotate;
        glm::mat4 mvp = m_Proj * m_View * model_positions[i];
        m_Shader->Bind();
        m_Shader->SetUniformMat4f("u_MVP", mvp);
        renderer.Draw(*m_VAO, *m_IB, *m_Shader);
    }
...
void Cube::OnRender(glm::mat4 proj, glm::mat4 view, std::vector<glm::mat4> model_positions, std::vector<float> scales)
{
    m_Proj = proj;
    m_View = view;
    GLCall(glClearColor(0.2f, 0.5f, 0.5f, 1.0f));
    ...
    for (int i = 0; i < model_positions.size(); i++)
    {
        glm::vec3 scale = glm::vec3(scales[i], scales[i], scales[i]);
        glm::mat4 model = glm::scale(model_positions[i], scale);
        glm::mat4 mvp = m_Proj * m_View * model;
        m_Shader->Bind();
        m_Shader->SetUniformMat4f("u_MVP", mvp);
        renderer.Draw(*m_VAO, *m_IB, *m_Shader);
